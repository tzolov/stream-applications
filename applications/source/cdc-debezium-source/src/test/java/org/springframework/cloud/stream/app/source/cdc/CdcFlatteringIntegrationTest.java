...
import static org.springframework.cloud.stream.app.source.cdc.CdcTestUtils.drain;
...
public class CdcFlatteringIntegrationTest {
	...
	final ContextConsumer<? super ApplicationContext> noFlatteringTest = context -> {
		...
		List<Message<?>> messages = drain(outputDestination);
		...
		messages = drain(outputDestination);
	...
	final ContextConsumer<? super ApplicationContext> flatteringTest = context -> {
		...
		if (flatteringProps.isEnabled() && flatteringProps.isOperationHeader()) {
			assertThat(messages.get(39).getHeaders().get("cdc_operation")).isEqualTo("c");
		}
		...
		if (flatteringProps.isOperationHeader()) {
			assertThat(messages.get(1).getHeaders().get("cdc_operation")).isEqualTo("u");
		}
		...
		if (flatteringProps.getDeleteHandlingMode() == CdcCommonProperties.DeleteHandlingMode.none) {
			...
			assertJsonEquals("{\"id\":" + newRecordId + "}", messages.get(1).getHeaders().get("cdc_key"));
			if (flatteringProps.isOperationHeader()) {
				assertThat(messages.get(2).getHeaders().get("cdc_operation")).isEqualTo("d");
			}
		}
